...
const ActionPanel: React.FC<{ tool: AnyTool; onClose: () => void; isMobile: boolean }> = ({ tool, onClose, isMobile: m }) => {
  ...
  return m ? (
    <><div onClick={onClose} style={{ position: "fixed", inset: 0, background: T.panelOverlay, zIndex: 900 }} />
    <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, maxHeight: "88vh", background: T.panelBg, borderRadius: "16px 16px 0 0", zIndex: 1000, overflowY: "auto", borderTop: `3px solid ${vc.color}`, boxShadow: "0 -4px 24px rgba(0,0,0,0.15)" }}>{content}</div></>
  ) : (
    <><div onClick={onClose} style={{ position: "fixed", inset: 0, background: T.panelOverlay, zIndex: 400 }} />
    <div style={{ position: "fixed", top: 0, right: 0, width: 380, height: "100vh", background: T.panelBg, borderLeft: `3px solid ${vc.color}`, zIndex: 500, overflowY: "auto", boxShadow: "-4px 0 24px rgba(0,0,0,0.1)" }}>{content}</div></>
  );
};
...
export default class GroupDashboard extends React.Component<IGroupDashboardProps, {
  ...
  public componentDidMount(): void { this._checkMobile(); window.addEventListener("resize", this._checkMobile); }
  public componentWillUnmount(): void { window.removeEventListener("resize", this._checkMobile); }
  private _checkMobile = (): void => { this.setState({ mob: window.innerWidth < 768 }); };
  ...
  public render(): React.ReactElement<IGroupDashboardProps> {
    ...
    return (
      <div style={{ fontFamily: "'Segoe UI', 'Segoe UI Web (West European)', -apple-system, sans-serif", background: T.bg, color: T.text, minHeight: 700, display: "flex", flexDirection: "column" }}>
